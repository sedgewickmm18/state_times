...
class State_Timer(BaseTransformer):
    ...
    def execute(self, df ):
        ...
        print ('Here', entity_index_name, time_index_name, simulation_data.columns)
        ...
        for asset in asset_list:
            ...
            for row in rows:
                #
                # Note - row[0] is entity, row[1] timestamp
                #    row[4] is currently hardcoded, we should find the correct entry from the order of the columns
                #
                if first_row == False:
                    logger.debug("Row")
                    logger.debug(row)
                    ...
                    for item in states:
                        ...
                        if row[4] == item:
                            ...
                            simulation_data.loc[
                                (simulation_data[entity_index_name] == asset) & (simulation_data[time_index_name] == row[1]), [
